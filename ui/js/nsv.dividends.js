var NSV = (function(NSV, $, undefined) {
	...
	NSV.div_send_calc_dividends = function () {
		...
		if (!(/^\d+$/.test(asset))) {
			...
			NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getAsset", {
				"asset": asset
			}, function(response) {
				if (response.errorCode) {
				...
				NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getTransaction", {"transaction": asset}, function(response) {
					if (response.errorCode) {
				...
				NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getTime", {}, function(response) {
					if (response.errorCode) {
				...
				NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getBlock", {"timestamp": present_time}, function(response) {
					if (response.errorCode) {
				...
				if (time_in !== "") {
					NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getBlock", {"timestamp": activate_timestamp}, function(response) {
						if (response.errorCode) {
							err_message = "Unknown error, couldn't getBlock.";
						} else {
				...
				if (list_nodist_acc !== "") {
					if (list_nodist_acc !== "ISSUER") {
						...
						for (i=0; i<account_array.length; ++i) {
							var act_acc = account_array[i];
							NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getAccount", {
								"account": act_acc,
                                "includeAssets" : true,
                                "includeLessors" : true,
				...
				if (mode_recent_block) {
					...
					NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getAssetAccounts", {
					...
					NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getTrades", {
						"asset": asset
					}, function(response) {
						if (response.errorCode) {
							...
							for (var i=0;i<trade_arr.length;i++) {
								if (trade_arr[i].timestamp <= activate_timestamp) {
									...
									NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getTransaction", {
	...
	NSV.div_send_get_transactions = function(cur_asset, asset_array,initial_time,final_time) {
		...
		for (var i=0; i<len; i++) {
			...
			NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getBlockchainTransactions", {
				"account":tmp_acc , "type":"2", "subtype":"1","timestamp":str_init_time,"includeExpiredPrunable":true
			}, function(response,input) {
				if (response.errorCode) {
					err_message2 = "Unknown bug. Problem accessing getBlockchainTransactions.";
					return;
				} else {
					...
					for (var j=0; j<tran_len; j++) {
						...
						NSV.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getTransaction", {
	...
	NSV.shareswap_calc = function() {
		...
		if (redeem_ac === "") {
			...
			NRS.sendOutsideRequest("/nxt?requestType=" + "getBlockchainTransactions", {
				"account":redeem_ac , "type":"2", "subtype":"1", "includeExpiredPrunable":true
			}, function(response,input) {
				if (response.errorCode) {
		...
		if (NSV_redeemed_assets.length === 0) {
			...
			for (var k=0; k<NSV_redeemed_assets.length; k++) {
				...
				NRS.sendOutsideRequest(NRS.server + "/nxt?requestType=" + "getBlockchainTransactions", {
					"account":NSV_redeemed_assets[k].sender , "type":"2", "subtype":"1","includeExpiredPrunable":true
				}, function(response,input) {
					if (response.errorCode) {
